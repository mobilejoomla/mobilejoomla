...
if(!defined('WURFL_USE_CACHE')) define('WURFL_USE_CACHE', $cache>0);
if(!defined('WURFL_USE_MULTICACHE')) define('WURFL_USE_MULTICACHE', $cache>1);
if(!defined('MAX_UA_CACHE')) define('MAX_UA_CACHE', $uacache);
...
function convertImage($imagepath,$scaletype=0)
{
	...
	elseif(strpos($imagepath,JURI::base())==0)
		$abs_imagepath=substr_replace($imagepath,JPATH_SITE,0,strlen(JURI::base())-1);
	else
		return $imagepath;
...
function getCAP($ua)
{
	...
	$bestFormat='';
	if ($devcap['image_format']['png'])
		$bestFormat='png';
	else if ($devcap['image_format']['jpg'])
		$bestFormat='jpg';
	else if ($devcap['image_format']['gif'])
		$bestFormat='gif';
	else
		$bestFormat='wbmp';
