...
?>
<form action="<?php echo JRoute::_('index.php');?>" method="post" data-ajax="true">
	<div class="ui-field-contain search<?php echo $moduleclass_sfx ?>">
		<?php
			$output = '<label for="mod-search-searchword">'.$label.'</label>'
					.'<input type="text" data-type="search" name="searchword" id="mod-search-searchword" maxlength="'.$maxlength.'" class="inputbox'.$moduleclass_sfx.'" value="'.($text==$label?'':$text).'" data-inline="true" />';
