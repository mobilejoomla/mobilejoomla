...
class AmddUA
{
    ...
    public static function normalize($ua)
    {
        ...
        $ua = preg_replace('#(/SN\d{15}|\[(ST|TF|NT)\d+\])#', '', $ua);
    ...
    public static function isDesktop($ua)
    {
        ...
        $linux_platforms   = '(Ubuntu; )?X11;( U;| Ubuntu;)? Linux[ ;]';
        $desktop_platforms = "(Macintosh; |(Windows; U; )?$windows_platforms|$linux_platforms)";
        ...
        // wget, php, java, etc
        if(preg_match('#^(AppEngine-Google|curl/|Java/|libwww-perl|Microsoft Office/|Outlook-Express/|PHP|Reeder/|Wget)#', $ua))
            return true;
    ...
    private static function _getGroup($ua)
    {
        ...
        if(preg_match('#Mozilla/5\.0 \(Linux; U?;? ?Android [^;]+; xx; ([^)]+?) Build/#', $ua, $match))
        {
            $model = $match[1];
