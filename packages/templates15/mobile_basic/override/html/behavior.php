...
class JHTMLBehavior
{
	...
	function mootools($debug = null)
	{
		...
		$mootoolsDir = JPluginHelper::isEnabled('system', 'mtupgrade') ? 'plugins/system/mtupgrade/' : 'media/system/js/';
		if ($debug || $konkcheck) {
			JHTML::script('mootools-uncompressed.js', $mootoolsDir, false);
		} else {
			JHTML::script('mootools.js', $mootoolsDir, false);
		}
		$loaded = true;
		return;
	}
	...
	function caption() {
		// Don't load in Elegance template
		return;
	}
