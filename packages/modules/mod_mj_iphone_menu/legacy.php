 ...
 */
function mosGetMenuLink_iphone($mitem, $level = 0, & $params, $open = null)
{
	...
	else
	{
		...
		else
		{
			...
			{
				if($mitem->type == 'url' && ItemidContained_iphone($mitem->link, $current_itemid))
				{
					$id = 'id="active_menu'.$params->get('class_sfx').'"';
				}
				else
 ...
 */
function mosShowIPHONEMenu(&$params)
{
	/** @var JParameter $params */
	...
	$hasSub = true;
	if($is_joomla16)
		$rows = $menu->getItems('parent_id', $activeId);
	else
		$rows = $menu->getItems('parent', $activeId);

	if(!is_array($rows) || $MobileJoomla->_ishomepage)
	{
		$hasSub = false;
	...
	if(is_array($rows) && count($rows))
	{
		...
		foreach($rows as $row) if($row->access <= $aid)
		{
			...
			{
				$links[] = mosGetMenuLink_iphone($row, 0, $params);
			}
		}
	}
 ...
 */
function ItemidContained_iphone($link, $Itemid)
{
