 ...
 */
function mosGetMenuLink_imode($mitem, $level = 0, & $params, $open = null)
{
	...
	else
	{
		...
		else
		{
			...
			{
				if($mitem->type == 'url' && ItemidContained_imode($mitem->link, $current_itemid))
				{
					$id = 'id="active_menu'.$params->get('class_sfx').'"';
				}
				else
 ...
 */
function mosShowIMODEMenu(&$params)
{
	/** @var JParameter $params */
	/** @var JSiteMenu $menu */
	$menu = & JSite::getMenu();
 ...
 */
function ItemidContained_imode($link, $Itemid)
{
