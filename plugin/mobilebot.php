...
class plgSystemMobileBot extends JPlugin
{
	...
	function onAfterInitialise()
	{
		...
		// don't filter RSS
		if (JRequest::getVar ('type', FALSE) == 'rss' || JRequest::getVar ('type', FALSE) == 'atom')
			return;
		...
		if(!$ismobile)
		{
			...
			{//Check for mobile device using selected method
				...
				{
					$wurflpath=JPATH_SITE.DS.'administrator'.DS.'components'.DS.'com_mobilejoomla'.DS.'terawurfl'.DS;
					$wurfl = is_file($wurflpath.'tera.sql');
					if(version_compare(PHP_VERSION, '5.0.0', '<'))
					{//Use CompactWURFL if WURFL isn't loaded
						$m=3;
						$MobileJoomla_Settings['useragent']=3;
						if($MobileJoomla_Settings['tmpl_xhtml_img']>1) $MobileJoomla_Settings['tmpl_xhtml_img']=0;
						if($MobileJoomla_Settings['tmpl_wap_img']>1) $MobileJoomla_Settings['tmpl_wap_img']=0;
						if($MobileJoomla_Settings['tmpl_imode_img']>1) $MobileJoomla_Settings['tmpl_imode_img']=0;
						if($MobileJoomla_Settings['tmpl_iphone_img']>1) $MobileJoomla_Settings['tmpl_iphone_img']=0;
					}
				}
				if(isset($methods[$m]))
